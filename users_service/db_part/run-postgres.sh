...
if docker run --name postgres-db --network ${NETWORK} -e POSTGRES_USER=test_user -e POSTGRES_DB=users_db -e POSTGRES_PASSWORD=1234 -p 5432:5432 -v "$(pwd)"/create_tables.sql:/docker-entrypoint-initdb.d/create_tables.sql -d postgres:latest; then
